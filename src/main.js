...
function createTodoEntryAndDescription(){
  ...
  if(myTemporaryTodoListEntryAndDescription.textInputTodoListEntry != ''){
    ...
    let alreadyExists = document.getElementById(`todoEntry${myTemporaryTodoListEntryAndDescription.textInputTodoListEntry}`);
    if(alreadyExists){
      alreadyExists = alreadyExists.id.replace('todoEntry', '');
    }
    if(alreadyExists != myTemporaryTodoListEntryAndDescription.textInputTodoListEntry){
      ...
      alert('Diesen Eintrag gibt es schon in deiner Todo-Liste.');
    }
  }else{
